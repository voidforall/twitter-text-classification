...
# Apply Roberta tokenizer for tokenization
max_len = 50
...
# model = RobertaForSentiment.from_pretrained('roberta-base')
model = RobertaForSentiment.from_pretrained('./saved/roberta_full/')
model = model.cuda()
...
def accuracy(preds, labels):
	...
	"""
	pred_round = np.argmax(preds, axis=-1)[:, np.newaxis]
...
output_dir = './saved/roberta_full_further/'
